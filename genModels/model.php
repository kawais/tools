...
$db['host'] = getInput('DataBase Host[192.168.18.18:5536]', '192.168.18.18:5536');
$db['user'] = getInput('DataBase User[root]', 'root');
$db['pass'] = getInput('DataBase Pass[root]', 'root');
$db['db'] = getInput('DataBase Name[heyshow_dsp_v3]', 'heyshow_dsp_v3');
$skfile = getInput('Skeleton file[skeletonbase.php]', 'skeletonbase.php');
...
$skeleton=file_get_contents($skfile);
while ($table = $result->fetch_assoc()) {
    $tableName = array_shift($table);
    $spaceName = convertUnderline($tableName);
    $modelName = str_replace('Dsp', '',$spaceName.'BaseModel');
    ...
    //生成基础模型文件
    $str = str_replace($search, $replace, $skeleton);
